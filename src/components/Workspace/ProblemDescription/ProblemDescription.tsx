...
const ProblemDescription: React.FC<ProblemDescriptionProps> = ({
  ...
  const retrieveUserAndProblemData = async (transaction: any) => {
  ...
  const handleDislikeClick = async () => {
    ...
    await runTransaction(firestore, async (transaction) => {
      ...
      if (userDoc.exists() && problemDoc.exists()) {
        // Already disliked a problem, already liked, or not disliked or liked
        if (disliked) {
          transaction.update(userRef, {
            dislikedProblems: userDoc
              .data()
          ...
          <div className="w-full">
            ...
            {!loading && currentProblem && (
              <div className="flex items-center mt-3">
                ...
                <div
                  className="cursor-pointer hover:bg-dark-fill-3  rounded p-[3px]  ml-4 text-xl transition-colors duration-200 
